...
hook in it resolves to `language: unsupported` — pre-commit provisions an interpreter of its own
for every other language, ignoring the one this repo pins. `unsupported` rather than its deprecated
alias `system`, which pre-commit rewrites on load and will eventually drop. Both cover every hook
regardless of which repo declares it, since `repo:` names where a hook's implementation lives
rather than what it checks; it decides only what an *absent* `language` means, which the schema
defaults per repo kind. Neither is a check over the tree, so the assertions are the whole gate and
they ride `bazel test //...` instead of costing a CI job.
...
settings rather than with the commit, which is why it runs as `security.yml`'s
`Repository constraint enforcement manifest consistency check` and on that workflow's Monday cron.
It compares the manifest against the rules GitHub reports for `main` and fails on any disagreement
in either direction, naming the contexts that moved rather than printing two lists. Every
